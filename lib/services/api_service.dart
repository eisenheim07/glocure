...
class ApiService {
  ...
  Future<List<ShopifyOrder>> getCustomerOrdersByStatus(String customerId, String status) async {
    ...
    try {
      // Use REST API with status parameter - Updated URL and access token
      ...
      AppLogger.info('🔍 API: Requesting orders with status="$status" for customer="$customerId"');
      ...
      final response = await http.get(
        Uri.parse(url),
        headers: {
          'X-Shopify-Access-Token': ApiConfig.shopifyAdminAccessToken, // Use ApiConfig instead of hardcoded token
          'Content-Type': 'application/json',
        },
      );
      ...
      if (response.statusCode == 200) {
        ...
        final orders = ordersData.map((orderData) {
          // Debug: Log the actual status values from API
          print(
              '🔍 API Order ${orderData['name']} - Financial: "${orderData['financial_status']}", Fulfillment: "${orderData['fulfillment_status']}"');
  ...
  /// Cancel an order from Shopify Admin API (REST)
  /// [orderId] - The Shopify order ID to cancel
  Future<bool> cancelOrder(String orderId) async {
    AppLogger.info('ApiService: Cancelling order: $orderId');

    try {
      final url = 'https://bxaqgp-p1.myshopify.com/admin/api/2025-10/orders/$orderId/cancel.json';
      ...
      AppLogger.info('ApiService: CANCEL order response - Status: ${response.statusCode}');

      if (response.statusCode == 200) {
        AppLogger.success('ApiService: Successfully cancelled order: $orderId');
        return true;
      } else {
        AppLogger.error('ApiService: Failed to cancel order - Status: ${response.statusCode}, Body: ${response.body}');
        throw Exception('Failed to cancel order: HTTP ${response.statusCode}');
      }
    } catch (e) {
      AppLogger.error('ApiService: Failed to cancel order $orderId: $e');
      rethrow;
    }
  }
