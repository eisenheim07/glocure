...
class OrderScreenState extends State<OrderScreen> with SingleTickerProviderStateMixin {
  ...
  void _fetchDataForTab(int index) {
    ...
    switch (index) {
      case 0:
        cubit.showPendingTab(); // Changed from showAllTab
        break;
      case 1:
        cubit.showClosedTab(); // Changed from showPendingTab
        break;
      case 2:
        cubit.showCancelledTab(); // Changed from showClosedTab
        break;
    }
  }

  Future<bool> _onWillPop() async {
    // If not on "Pending" tab (first tab), go to "Pending" tab first
    if (_currentIndex != 0) {
      _onTabTapped(0);
      return false; // Don't pop the route
    }
    // If on "Pending" tab, allow back navigation
    return true;
  }
      ...
      child: Scaffold(
        ...
        appBar: CustomAppBar(
          ...
          onBackPressed: () {
            ...
            if (_currentIndex != 0) {
              // If not on "Pending" tab, go to "Pending" tab
              _onTabTapped(0);
            } else {
              // If on "Pending" tab, go back to home
          ...
          return RefreshIndicator(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return _OrderCard(order: state.orders[index], currentTab: state.currentTab);
              },
            ),
          );
        }
...
class _OrderCard extends StatefulWidget {
  ...
  const _OrderCard({required this.order, required this.currentTab});
...
class _OrderCardState extends State<_OrderCard> {
          ...
          Row(
            children: [
              ...
              Container(
                ...
                child: Text(
                  _getStatusText(widget.order, widget.currentTab),
                  style: TextStyle(
                    fontSize: 11.fSize,
                    fontWeight: FontWeight.w600,
                    color: _getStatusColor(widget.order, widget.currentTab),
                    fontFamily: 'Inter',
                  ),
                ),
              ),
              PopupMenuButton<String>(
                ...
                onSelected: (value) {
                  if (value == 'view_details') {
                    ...
                    _onCancelOrder(context, widget.order);
                ...
                itemBuilder: (BuildContext context) => [
                  ...
                  // Only show Track Order for non-cancelled orders
                  if (widget.currentTab.toLowerCase() != 'cancelled')
                    PopupMenuItem<String>(
                      value: 'track_order',
                      child: Row(
                        children: [
                  ...
                  // Only show Cancel Order for pending orders
                  if (widget.currentTab.toLowerCase() == 'pending')
                    PopupMenuItem<String>(
                      value: 'cancel_order',
                      child: Row(
                        children: [
  ...
  void _onTrackOrder(BuildContext context, ShopifyOrder order) {
    // Try WebView first, but provide fallback to external browser
    Navigator.push(
      context,
      MaterialPageRoute(
  ...
  void _onCancelOrder(BuildContext context, ShopifyOrder order) {
    // Show confirmation bottom sheet for cancelling order
    CommonBottomSheet.show(
      context: context,
      title: 'Cancel Order',
      message: 'Are you sure you want to cancel order ${order.name}? This action cannot be undone.',
      icon: Icon(
        Icons.cancel_outlined,
        size: 44.h,
        color: const Color(0xFFFF5C9A),
      ...
      onPrimaryPressed: () async {
        ...
        try {
          // Call cubit to cancel order (this will show loading state and refresh)
          ...
          if (context.mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Order ${order.name} cancelled successfully'),
  ...
  Color _getStatusColor(ShopifyOrder order, String currentTab) {
    // Hardcode color based on current tab
    switch (currentTab.toLowerCase()) {
      case 'pending':
        return const Color(0xFFFF9800); // Orange for pending
      case 'closed':
        return const Color(0xFF4CAF50); // Green for closed/delivered
      case 'cancelled':
        return const Color(0xFFF44336); // Red for cancelled
      default:
        return const Color(0xFF2196F3); // Blue for processing
    }
  }

  String _getStatusText(ShopifyOrder order, String currentTab) {
    // Hardcode status text based on current tab
    switch (currentTab.toLowerCase()) {
      case 'pending':
        return 'Pending';
      case 'closed':
