...
class PaymentStatusScreen extends StatefulWidget {
  ...
  final bool isFromSingleProduct; // Flag to indicate if this is from single product purchase
  ...
  const PaymentStatusScreen({
    ...
    this.isFromSingleProduct = false, // Default to false for backward compatibility
  });
...
class _PaymentStatusScreenState extends State<PaymentStatusScreen> {
  ...
  void initState() {
    ...
    // Only generate new cart ID if payment is successful AND it's not from single product purchase
    if (widget.status.toLowerCase() == 'success' && !widget.isFromSingleProduct) {
      // For cart-based purchases, generate new cart ID
      context.read<PaymentStatusCubit>().generateNewCartId(skipCartGeneration: false);
    } else if (widget.status.toLowerCase() == 'success' && widget.isFromSingleProduct) {
      // For single product purchases, just show shimmer without GraphQL call
      context.read<PaymentStatusCubit>().generateNewCartId(skipCartGeneration: true);
    }
    // For failed/cancelled payments, don't generate cart ID at all
  }
